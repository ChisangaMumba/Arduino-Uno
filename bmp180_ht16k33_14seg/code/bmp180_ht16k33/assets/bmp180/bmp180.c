...
//------------------------------------------------------------------------------
void bmp180_write(uint8_t reg, uint8_t data)
{
  i2c_start();                      
...
uint16_t bmp180_readUncompTemp(void)
{
	...
	delay(5);
	data[1] = bmp180_read8(MSB);   
	data[0] = bmp180_read8(LSB);   
...
float bmp180_getTemp(void)
{
  ...
  return tmp * 0.1; // Convert to °C
}
//------------------------------------------------------------------------------
uint32_t  bmp180_readUncompPress(uint8_t OSS = OSS_0) // default value for oversampling setting
{
  uint32_t data[3];
...
//  see page 14 of BMP180 datasheet 15 (Algorithm for pressure and temperature measurement)             
//------------------------------------------------------------------------------
float bmp180_getPressure(uint8_t OSS = OSS_0) // default value for oversampling setting
{
