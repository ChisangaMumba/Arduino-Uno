...
{ 
  ...
  { 
    ...
    if(isprint(chr) && chr != '.' && (chr != 176 || chr != 194)) // If the character is printable and not a decimal point or degree symbol
      chip++;
  }
}
...
void writeChar(uint8_t chr, uint8_t chip)
{
  ...
  for(col = 0; col < 5; col++)
  {
    ...
    {
      row = chr - ' '; // Calculate the row index based on the ASCII value of the character
