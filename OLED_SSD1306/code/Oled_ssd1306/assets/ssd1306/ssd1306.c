...
void writeChar(uint8_t chr)
{
	...
	for(col = 0; col < 5; col++)
	{
		if(chr >= 32 && chr <= 127)									//Standard ascii Printable chars range 32 - 127 or 0 - 95 (after subtraction). degree symbol num � is 144 after subtraction
			ssd1306_data( chars[chr - ' '][col] );		//(' ' = 32) chr - 32...because the first 32  ASCII character codes are none Printable (control chars) 
		else
			ssd1306_data( customChars[0][col] ); 			//Custom Char is ° symbol
	}
