...
void SHT7x_init()
{
	pinMode(SHT_CLK, OUTPUT);
}
//------------------------------------------------------------------------------
void SBus_start(void) 
{
	pinMode(SHT_DATA, INPUT);		// SHT_DATA = 1
	digitalWrite(SHT_CLK, HIGH);	// SHT_CLK  = 1
	delayMicroseconds(5);
	pinMode(SHT_DATA, OUTPUT);		// SHT_DATA = 0
	delayMicroseconds(5);
...
{	
	...
	pinMode(SHT_DATA, (ack) ? INPUT : OUTPUT);		// SHT_DATA = ack;
	digitalWrite(SHT_CLK, HIGH);					// SHT_CLK = 1
...
uint8_t SBus_write(uint8_t data) 
{
	...
	{
		pinMode(SHT_DATA, ((data << tx) & 0x80) ? INPUT : OUTPUT);  // MSB first
...
uint8_t SBus_waitForResult(void) 
{
	...
	pinMode(SHT_DATA, INPUT);				// SHT_DATA = 1 Release DATA (input mode)
...
// According to the SHT7x datasheet, the temperature conversion is given by:
...
// Note: Coefficients may vary ° consult the SHT7x datasheet for your sensor.
//------------------------------------------------------------------------------
