...
void ds18b20_init(uint8_t highTemp, uint8_t lowTemp, uint8_t res)
{
  ...
  ds18b20_writeByte(highTemp);
  ds18b20_writeByte(lowTemp);
  ds18b20_writeByte(res);
}
//------------------------------------------------------------------------------
...
//------------------------------------------------------------------------------
uint8_t ds18b20_isDataValid(uint8_t data[])
{
	uint8_t isValid = 0;
	
	if(ds18b20_calcCRC(data) == data[8])        // data[8] == CRC value
	{
		isValid = 1;
	}
