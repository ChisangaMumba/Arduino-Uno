...
void bmp180_init(void)
{
  i2c_init();
	...
	ac1 = bmp180_readWord(AC1_reg);
	ac2 = bmp180_readWord(AC2_reg);
	ac3 = bmp180_readWord(AC3_reg);
	ac4 = bmp180_readWord(AC4_reg);
	ac5 = bmp180_readWord(AC5_reg);
	ac6 = bmp180_readWord(AC6_reg);
	_b1 = bmp180_readWord(B1_reg);
	_b2 = bmp180_readWord(B2_reg);
	mb = bmp180_readWord(MB_reg);
	mc = bmp180_readWord(MC_reg);
	md = bmp180_readWord(MD_reg);
}
//------------------------------------------------------------------------------
...
uint16_t bmp180_readUncompTemp(void)
{
	...
	delay(5);
	data[1] = bmp180_readByte(MSB);   
	data[0] = bmp180_readByte(LSB);   
...
uint32_t  bmp180_uncompPressure(uint8_t OSS)
{
  ...
  msb_value = bmp180_readByte(MSB);
  lsb_value = bmp180_readByte(LSB);
  xlsb_value = bmp180_readByte(XLSB);

  up = ((((uint32_t)msb_value << 16) | ((uint32_t)lsb_value << 8) | (uint32_t)xlsb_value) >> (8 - OSS));
...
{
  int32_t UP = bmp180_uncompPressure(OSS);
